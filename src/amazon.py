...
class AmazonReviewsScrapper:
    ...
    def __create_url__(self, url, options: dict, type='reviews'):
        ...
        reviews_url = "/".join(
            [*url_parts[:3], product_name, 'product-reviews', product_id])
    ...
    def __validate_options__(self, options):
        valid_options = {
            ...
            "filterByStar": [
                "all_stars", "one_star", "two_star", "three_star", "four_star",
                "five_star", "positive", "critical"
            ],
        ...
        for key, valid_values in valid_options.items():
            if options[key] not in valid_values:
                raise InvalidOptionError(
                    f"{options[key]} is not a valid {key} option. Valid options are {valid_values}"
                )
    ...
    def __get_total_reviews__(self):
        try:
            total_reviews_block = self.browser.find_element(
                By.CSS_SELECTOR, 'div#filter-info-section')
            total_reviews_html = self.browser.execute_script(
                "return arguments[0].outerHTML;", total_reviews_block)
        except StaleElementReferenceException:
            raise StaleElementReferenceException(
                "Error in accessing total reviews block")
        except NoSuchElementException:
            raise NoSuchElementException("Total reviews block not found")
        soup = BeautifulSoup(total_reviews_html, 'html.parser')
        reviews_text = soup.find(
            'div', {
    ...
    def __get_product_name__(self):
        try:
            product_name = self.browser.find_element(
                By.XPATH,
                '//*[@id="cm_cr-product_info"]/div/div[2]/div/div/div[2]/div[1]/h1/  a'
            )
            return str(product_name.get_attribute('innerHTML'))
        except StaleElementReferenceException:
            raise StaleElementReferenceException(
                "Error in accessing product name")
        except NoSuchElementException:
            raise NoSuchElementException("Product name not found")

    def __get_average_rating__(self):
        try:
            avg_rating_block = self.browser.find_element(
                By.XPATH,
            ...
            raise StaleElementReferenceException(
                "Error in accessing average rating block")
        except NoSuchElementException:
            raise NoSuchElementException("Average rating block not found")
        soup = BeautifulSoup(avg_rating_html, 'html.parser')
        avg_rating_text = soup.find('span', {
    ...
    def __get_total_ratings__(self):
        try:
            total_ratings = self.browser.find_element(
                By.XPATH,
                '//*[@id="cm_cr-product_info"]/div/div[1]/div[3]/span')
            total_ratings = total_ratings.get_attribute('innerHTML')
        except StaleElementReferenceException:
            raise StaleElementReferenceException(
                "Error in accessing total ratings")
        except NoSuchElementException:
            raise NoSuchElementException("Total ratings not found")
        total_ratings = int(re.search(r'(\d+)', str(total_ratings))[1])
        return total_ratings

    def __get_individual_ratings__(self):
        try:
            histogram_block = self.browser.find_element(
                By.XPATH, '//*[@id="histogramTable"]/tbody')
            histogram_block_html = histogram_block.get_attribute('innerHTML')
        except StaleElementReferenceException:
            raise StaleElementReferenceException(
                "Error in accessing histogram block")
        except NoSuchElementException:
            raise NoSuchElementException("Histogram block not found")
        soup = BeautifulSoup(histogram_block_html, 'html.parser')
        stars_data = {}
        for row in soup.select('tr.a-histogram-row'):
            star_rating = (
                row.select_one('td.aok-nowrap span.a-size-base')
                or row.select_one('td.a-nowrap span.a-size-base')).get_text(
                    strip=True)
            percentage = row.select_one(
                'td.a-text-right span.a-size-base').get_text(strip=True)
            stars_data[star_rating] = int(percentage.rstrip('%'))
        # Calculate absolute number of ratings for each star
        stars_data = {
            star: round((percentage / 100) * self.__get_total_ratings__())
            for star, percentage in stars_data.items()
        }
        return stars_data
        ...
        def fetch_reviews():
            try:
                return [
                    ...
                    for review in self.browser.find_elements(
                        By.CSS_SELECTOR, '[id^="customer_review-"]')
                ]
            except StaleElementReferenceException:
                raise StaleElementReferenceException(
                    "Error in accessing reviews")
            except NoSuchElementException:
                raise NoSuchElementException("Reviews not found")
    ...
    def __get_reviews__(self):
        ...
        return [
            self.__parse_review__(review_html) for review_html in reviews_html
        ]

    def __parse_review__(self, review_html):
        review_parser = BeautifulSoup(review_html, 'html.parser')
        review_element = review_parser.find(
        ...
        reviewer_name = review_element.find(
            'span', class_='a-profile-name').get_text(strip=True)
        review_location, review_date = self.__extract_country_and_date__(
            review_element.find('span', attrs={
                'data-hook': 'review-date'
            }).get_text(strip=True))
        rating = int(
            float(
                review_element.find('span', class_='a-icon-alt').get_text(
                    strip=True).rstrip(" out of 5 stars")))
        review_title = review_element.find(
            'a', attrs={
                'data-hook': 'review-title'
            }).find('span', class_=False).get_text(strip=True)
        review_body = review_element.find('span',
                                          attrs={
                                              'data-hook': 'review-body'
                                          }).find('span',
                                                  class_=False,
                                                  recursive=False)
        review_content = (review_body.get_text(
            strip=True) if review_body is not None else '')

        media = [
            video_block.get('data-video-url')
            for video_block in review_element.find_all(
                'div', id=lambda x: x and x.startswith('review-video-'))
        ]
